...
export const createCIDTask = async (taskData) => {
  ...
  if (!task_approver_id) throw new Error(`The approver name: ${taskData.approver_name} not found`);
  ...
  const query = `
    INSERT INTO cid_task (task_category_id, cid_id, status, assignee_id, deadline, task_approver_id)
    VALUES ($1, $2, COALESCE($3, 'in-progress'), $4, $5, $6)
    RETURNING *`;
...
export const getAllCIDTasks = async () => {
  const query = `
    SELECT 
      ...
      ct.approval_date
    FROM cid_task ct
    JOIN user u1 ON ct.assignee_id = u1.user_id
    LEFT JOIN user u2 ON ct.task_approver_id = u2.user_id
...
export const updateCIDTaskStatus = async (cidTaskId, newStatus) => {
  const query = `
    UPDATE cid_task 
    SET status = $1, submitted_date = CASE WHEN $1 = 'submitted' THEN CURRENT_TIMESTAMP ELSE submitted_date END
    WHERE cid_task_id = $2
    RETURNING *`;
