import React, { useContext } from 'react'; import { AuthContext } from '../context/AuthContext.jsx'; import MainLayout from '../components/Layout/MainLayout.jsx'; import { Box, Typography, Paper, Grid, Card, CardContent } from '@mui/material'; import { PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, LabelList, Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis } from 'recharts';

const pieData = [ { name: 'America', value: 43.8 }, { name: 'Asia', value: 31.3 }, { name: 'Europe', value: 18.8 }, { name: 'Africa', value: 6.3 }, ];

const pieColors = ['#007bff', '#fbbc04', '#6f42c1', '#ea4335'];

const barData = [ { name: 'Jan', TeamA: 40, TeamB: 50 }, { name: 'Feb', TeamA: 30, TeamB: 70 }, { name: 'Mar', TeamA: 20, TeamB: 45 }, { name: 'Apr', TeamA: 35, TeamB: 65 }, { name: 'May', TeamA: 65, TeamB: 40 }, { name: 'Jun', TeamA: 65, TeamB: 35 }, { name: 'Jul', TeamA: 40, TeamB: 30 }, { name: 'Aug', TeamA: 25, TeamB: 70 }, { name: 'Sep', TeamA: 55, TeamB: 25 }, ];

const radarData = [ { subject: 'Math', A: 120, B: 110, C: 95 }, { subject: 'English', A: 98, B: 130, C: 85 }, { subject: 'History', A: 86, B: 90, C: 100 }, { subject: 'Physics', A: 99, B: 100, C: 110 }, { subject: 'Geography', A: 85, B: 95, C: 80 }, { subject: 'Chinese', A: 65, B: 70, C: 75 }, ];

const horizontalData = [ { name: 'Italy', A: 44, B: 53 }, { name: 'Japan', A: 32, B: 55 }, { name: 'China', A: 38, B: 41 }, { name: 'Canada', A: 50, B: 64 }, { name: 'France', A: 13, B: 27 }, ];

const DashboardPage = () => { const { isAuthenticated } = useContext(AuthContext);

return ( <MainLayout> <Box sx={{ padding: 4 }}> <Typography variant="h4" gutterBottom> Hi, Welcome back 👋 </Typography>
